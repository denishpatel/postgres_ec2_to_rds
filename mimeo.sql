...
CREATE TABLE dblink_mapping (
    data_source_id integer NOT NULL DEFAULT nextval('dblink_mapping_data_source_id_seq'),
...
ALTER TABLE refresh_config_snap ADD COLUMN post_script text[];
ALTER TABLE refresh_config_snap ALTER COLUMN type SET DEFAULT 'snap';
...
ALTER TABLE refresh_config_inserter ADD COLUMN control text NOT NULL;   
ALTER TABLE refresh_config_inserter ADD COLUMN boundary interval;
ALTER TABLE refresh_config_inserter ADD COLUMN dst_active boolean NOT NULL DEFAULT true;
ALTER TABLE refresh_config_inserter ADD COLUMN dst_start int NOT NULL DEFAULT 30;
ALTER TABLE refresh_config_inserter ADD COLUMN dst_end int NOT NULL DEFAULT 230;
ALTER TABLE refresh_config_inserter ALTER COLUMN type SET DEFAULT 'inserter';
...
ALTER TABLE refresh_config_updater ADD COLUMN control text NOT NULL;  
ALTER TABLE refresh_config_updater ADD COLUMN boundary interval;
ALTER TABLE refresh_config_updater ADD COLUMN pk_field text[] NOT NULL; 
ALTER TABLE refresh_config_updater ADD COLUMN pk_type text[] NOT NULL;
ALTER TABLE refresh_config_updater ADD COLUMN dst_active boolean NOT NULL DEFAULT true;
ALTER TABLE refresh_config_updater ADD COLUMN dst_start int NOT NULL DEFAULT 30;
ALTER TABLE refresh_config_updater ADD COLUMN dst_end int NOT NULL DEFAULT 230;
ALTER TABLE refresh_config_updater ALTER COLUMN type SET DEFAULT 'updater';
...
ALTER TABLE refresh_config_dml ADD COLUMN control text NOT NULL;  
ALTER TABLE refresh_config_dml ADD COLUMN pk_field text[] NOT NULL; 
ALTER TABLE refresh_config_dml ADD COLUMN pk_type text[] NOT NULL;
ALTER TABLE refresh_config_dml ALTER COLUMN type SET DEFAULT 'dml';
...
ALTER TABLE refresh_config_logdel ADD COLUMN control text NOT NULL;  
ALTER TABLE refresh_config_logdel ADD COLUMN pk_field text[] NOT NULL; 
ALTER TABLE refresh_config_logdel ADD COLUMN pk_type text[] NOT NULL;
ALTER TABLE refresh_config_logdel ALTER COLUMN type SET DEFAULT 'logdel';
ALTER TABLE refresh_config_logdel ADD CONSTRAINT refresh_config_logdel_type_check CHECK (type = 'logdel');
...
CREATE FUNCTION auth(integer) RETURNS text
    LANGUAGE sql
    AS $$
    select data_source||' user='||username||' password='||pwd from dblink_mapping where data_source_id = $1; 
$$;
...
BEGIN
     ...
     SELECT post_script INTO v_post_script FROM refresh_config WHERE dest_table = p_dest_table;
...
EXECUTE 'SELECT set_config(''search_path'','''||v_jobmon_schema||','||v_dblink_schema||''',''false'')';
...
IF v_rowcount IS NOT NULL THEN
    ...
    IF v_match = 'f' AND v_post_script IS NOT NULL THEN
        ...
        PERFORM post_script(v_dest_table);
        PERFORM update_step(v_step_id, 'OK','Done');
    END IF;
 ...
 */
CREATE FUNCTION refresh_inserter(p_destination text, p_debug boolean, p_limit integer DEFAULT 100000) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
ELSE
    SELECT array_to_string(array_agg(attname),','), array_to_string(array_agg(attname||' '||atttypid::regtype::text),',') FROM 
        (SELECT unnest(filter) FROM refresh_config_inserter WHERE dest_table = p_destination) x 
 ...
 */
CREATE FUNCTION refresh_updater(p_destination text, p_debug boolean, p_limit integer DEFAULT 100000) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE FUNCTION refresh_dml(p_destination text, p_debug boolean, p_limit int default 100000) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
 ...
 */
CREATE FUNCTION refresh_logdel(p_destination text, p_debug boolean, p_limit int default 100000) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
...
SELECT data_source INTO v_data_source FROM dblink_mapping WHERE data_source_id = p_dblink_id; 
IF NOT FOUND THEN
	RAISE EXCEPTION 'Database link ID does not exist in dblink_mapping: %', p_dblink_id; 
END IF;  

v_insert_refresh_config := 'INSERT INTO refresh_config_snap(source_table, dest_table, dblink) VALUES('
    ||quote_literal(p_src_table)||', '||quote_literal(p_src_table)||', '|| p_dblink_id||');';

RAISE NOTICE 'Inserting record in refresh_config';
EXECUTE v_insert_refresh_config;	
RAISE NOTICE 'Insert successful';	

RAISE NOTICE 'attempting first snapshot';
PERFORM refresh_snap(p_src_table, FALSE);
...
SELECT data_source INTO v_data_source FROM dblink_mapping WHERE data_source_id = p_dblink_id; 
IF NOT FOUND THEN
	RAISE EXCEPTION 'ERROR: Database link ID does not exist in dblink_mapping: %', p_dblink_id; 
END IF;  
...
RAISE NOTICE 'attempting first snapshot';
PERFORM refresh_snap(p_dest_table, FALSE);
...
SELECT source_table, dest_table INTO v_src_table, v_dest_table
    FROM refresh_config_snap WHERE dest_table = p_dest_table;
IF NOT FOUND THEN
...
EXECUTE 'DELETE FROM refresh_config_snap WHERE dest_table = ' || quote_literal(v_dest_table);
...
v_dst_active := dst_utc_check();

v_insert_refresh_config := 'INSERT INTO refresh_config_inserter(source_table, dest_table, dblink, control, boundary, last_value, dst_active) VALUES('
    ||quote_literal(p_src_table)||', '||quote_literal(p_src_table)||', '|| p_dblink_id||', '
...
EXECUTE 'DELETE FROM refresh_config_snap WHERE source_table = '||quote_literal(p_src_table)||' AND dest_table = '||quote_literal(p_src_table);
...
-- Remove temp snap from config
EXECUTE 'DELETE FROM refresh_config_snap WHERE source_table = '||quote_literal(p_src_table)||' AND dest_table = '||quote_literal(p_dest_table);
...
SELECT source_table, dest_table INTO v_src_table, v_dest_table
		FROM refresh_config_inserter WHERE dest_table = p_dest_table;
IF NOT FOUND THEN
...
EXECUTE 'DELETE FROM refresh_config_inserter WHERE dest_table = ' || quote_literal(v_dest_table);	
...
v_insert_refresh_config := 'INSERT INTO refresh_config_updater(source_table, dest_table, dblink, control, boundary, pk_field, pk_type, last_value, dst_active) VALUES('
...
SELECT source_table, dest_table INTO v_src_table, v_dest_table
    FROM refresh_config_updater WHERE dest_table = p_dest_table;
IF NOT FOUND THEN
...
EXECUTE 'DELETE FROM refresh_config_updater WHERE dest_table = ' || quote_literal(v_dest_table);
